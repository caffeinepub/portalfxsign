...
export default function PlanProfitPage() {
  const { startTransition } = useRouteTransition();
  const search = useSearch({ strict: false }) as { plan?: string };
  const [planTier, setPlanTier] = useState<string>('regular');

  // Resolve plan tier from multiple sources with safe default
  useEffect(() => {
    const resolvedPlan = 
      search.plan || 
      sessionStorage.getItem('profitPagePlanTier') || 
      'regular';
  ...
  const planMainProfit: Record<string, string> = {
    regular: '$5,000',
    vip: '$10,000',
    'vip-plus': '$15,000',
  };
  ...
  const currentPlanData = planData[planTier] || planData.regular;
  const currentPlanName = planNames[planTier] || planNames.regular;
  const currentMainProfit = planMainProfit[planTier] || planMainProfit.regular;
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold mb-3 bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-400 dark:to-teal-400 bg-clip-text text-transparent">
            Your {currentPlanName} Profit Overview
          </h1>
          <p className="text-lg text-muted-foreground">
            Explore your profit potential and exclusive product opportunities
          </p>
        </div>
        ...
        <GreenSectionCard className="p-8 mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-bold">Main Profit Potential</h2>
              <p className="text-muted-foreground">Your tier's base earning potential</p>
            </div>
          </div>
          <div className="text-center py-6 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg">
            ...
            <p className="text-sm text-muted-foreground mt-2">Base profit potential</p>
          </div>
        </GreenSectionCard>
        ...
        <GreenSectionCard className="p-8 mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-bold">Extra Profit Opportunities</h2>
              <p className="text-muted-foreground">Additional monthly earning potential</p>
            </div>
          </div>
          <div className="text-center py-6 bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 rounded-lg border-2 border-emerald-200 dark:border-emerald-800">
            <p className="text-4xl md:text-5xl font-bold text-emerald-600 dark:text-emerald-400">
              {currentPlanData.extraProfit}
            </p>
            <p className="text-sm text-muted-foreground mt-2">per month in extra profits</p>
          </div>
        </GreenSectionCard>
